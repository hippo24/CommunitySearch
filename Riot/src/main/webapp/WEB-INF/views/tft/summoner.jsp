            ...
            success: function(matchDetailResponse) {
                if (matchDetailResponse.error) {
                    ...
                    matchDetailResponse.info.participants.forEach(function(player) {
                        // 비동기 통신으로 전설이 정보 가져오기(비동기라 순서 무시하는거같음. 그래서 이렇게 전체 감싸봄)
                        fetch("https://ddragon.leagueoflegends.com/cdn/15.8.1/data/en_US/tft-tactician.json")
					    .then(res=>res.json())
					    .then(res=>{
		                    // 입력한 유저의 정보만 표시
	                        if (player.puuid === puuid) {
	                        	//player.companion //console.log(player.companion.item_ID);
	                        	
	                        	//전설이 이미지 url
						    	playerUrl = "https://ddragon.leagueoflegends.com/cdn/15.8.1/img/tft-tactician/" + res.data[player.companion.item_ID].image.full
